...
<script>
      ...
      if (accessToken) {
        ...
        .then(response => {
          if (response.status === 401) {
            if (response.statusText === '401-EXPIRED-TOKEN') {
        ...
        .then(data => {
          if (data && data.status === 'success') {
            ...
            const fingerprints = userDetails.FINGERPRINTS;
    ...
    function refreshToken() {
      return fetch('/api/auth/refresh-token', {
        ...
        credentials: 'include' // Include cookies containing the http-only CBSTORAGE_REFRESH_TOKEN
      })
      .then(response => response.json())
      .then(data => {
        if (data.status === 'success') {
          setAccessToken(data.detail.access_token, data.detail.expires_in);
